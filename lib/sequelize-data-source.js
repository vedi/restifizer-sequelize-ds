...
SequelizeDataSource.prototype.save = function save(doc) {
  ...
  return doc.save().
    then(function (newDoc) {
      var where = {};
      where[_this.idField] = newDoc[_this.idField];
      return _this.findOne({filter: where, fields: _this.fieldMap});
    });
};
...
SequelizeDataSource.prototype.proceedArrayMethod = function proceedArrayMethod(source, methodName, fieldName, scope) {
  // TODO: Implement
...
SequelizeDataSource.prototype._buildQ = function _buildQ(qFields, q) {
  return _.collect(qFields, function (qField) {
    return [qField + ' LIKE "%' + q + '%"'];
  });
};
