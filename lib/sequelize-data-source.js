...
SequelizeDataSource.prototype.find = function find(options) {
  ...
  return this.ModelClass.findAll(query).
    then(function(docs) {
...
SequelizeDataSource.prototype.findOne = function findOne(options) {
  var filter = options.filter;
  var fields = options.fields;
  var queryPipe = options.queryPipe;
  ...
  SequelizeDataSource.prototype.create = function create(data) {
    return this.ModelClass.build(data);
  };

  SequelizeDataSource.prototype.save = function save(doc) {
    var _this = this;
    return doc.save().
      then(function (newDoc) {
        return _this.findOne({filter: newDoc[_this.idField], fields: _this.fieldMap});
      });
  };

  SequelizeDataSource.prototype.remove = function remove(doc) {
    return doc.destroy().then(function () {
      return doc;
    });
  };

  SequelizeDataSource.prototype.count = function count(options) {
    var filter = options.filter;
    var q = options.q;
    var qFields = options.qFields;
  ...
  SequelizeDataSource.prototype.proceedArrayMethod = function proceedArrayMethod(methodName, fieldName, scope) {
    // TODO: Implement
    //// get sure we have an array
    //if (scope.dest[fieldName] === undefined) {
    //  scope.dest[fieldName] = [];
    //}
    //
    //if (methodName === '$addToSet') {
    //  scope.dest[fieldName].addToSet(scope.source);
    //} else if (methodName === '$pop') {
    //  if (scope.source === 1) {
    //    scope.dest[fieldName].pop();
    //  } else if (scope.source === -1) {
    //    scope.dest[fieldName].shift();
    //  } else {
    //    throw new Error('Illegal param value for $pop method');
    //  }
    //} else if (methodName === '$push') {
    //  scope.dest[fieldName].push(scope.source);
    //} else if (methodName === '$pull') {
    //  scope.dest[fieldName].pull(scope.source);
    //}
  };

  SequelizeDataSource.prototype.assignField = function assignField(fieldName, scope) {
    var dest = scope.isInsert() ? scope.dest : scope.model;
  ...
  SequelizeDataSource.prototype._applyQ = function (q, qFields, filter) {
