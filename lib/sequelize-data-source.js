...
SequelizeDataSource.prototype.find = function find(options, callback) {
  ...
  var skip = options.page ? limit * (options.page - 1) : 0;
...
SequelizeDataSource.prototype.countAsync = function countAsync(filter) {
  this._normalizeFilter(filter, this.ModelClass);
  return this.ModelClass.count({where: filter});
};
...
SequelizeDataSource.prototype._normalizeFields = function _normalizeFields(fields, filter, query) {
  ...
  fields = this._resolveAssociations(this.ModelClass, this.fieldMap, fields, filter, query.include);

  if (!fields.indexOf(this.ModelClass.primaryKeyField) < 0) {
...
SequelizeDataSource.prototype._resolveAssociations = function (ModelClass, fieldMap, fields, filter, include) {
  include = include || [];
  var fieldsToRemove = [];
  var filterToRemove = [];
  var result = _.map(fields, function (field) {
    ...
    if (fieldMeta && association) {
      ...
      associationFields = this._resolveAssociations(association.target, fieldMeta.fields, associationFields, filterValue, nestedInclude);
